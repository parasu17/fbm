...
function cleaningTypesPage() {
	clearPage();
	createCleaningTypesPage();
}

function createCleaningTypesPage() {
	createButtons();
	tabulator = createClientsTable();
	getAllCleaningTypes();
}

function getAllCleaningTypes() {
	  var xhttp = new XMLHttpRequest();
	  xhttp.onreadystatechange = function() {
	    if (this.readyState == 4 && this.status == 200) {
	    	var fbmResponse = JSON.parse(this.responseText);
	    	if(fbmResponse.success) {
	    		refreshCleaningTypes(fbmResponse.responseData);
	    	} else {
	    		showAlert('Could not fetch all clients from server');
	    	}
	    }
	  };
	  xhttp.open("GET", "services/clients/getAllCleaningTypes", true);
	  xhttp.send();
}
...
function createButtons() {
	...
	var buttonTableStr = 
		'<table class="buttonTableClass">' +
			'<tr>' +
				'<td>' +
					'<button type="button" onclick="openAddCleaningTypeForm()">Add Cleaning Type</button>' +
				'</td>' +
				'<td>' +
					'<button type="button" onclick="getAllCleaningTypes(refreshCleaningTypes)">Refresh Cleaning Types</button>' +
				'</td>' +
			'</tr>' +
		'</table>';
...
function openAddCleaningTypeForm() {
	  var xhttp = new XMLHttpRequest();
	  xhttp.onreadystatechange = function() {
	    if (this.readyState == 4 && this.status == 200) {
	    	prepareClientTypesForAddClientForm('Add Cleaning Type', createDomElement(this.responseText));
	    }
	  };
...
// data handling
function refreshCleaningTypes(clientList) {
	clients = clientList;
